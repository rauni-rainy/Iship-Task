...
export const CountdownTimer: React.FC<CountdownTimerProps> = ({ targetDate, onExpire }) => {
  ...
  return (
    <div className="flex gap-2 font-mono text-2xl font-bold tracking-tight text-slate-900 dark:text-slate-100">
      {timeLeft.days > 0 && (
        <>
      ...
      <div className="flex flex-col items-center">
        ...
        <span className="text-xs font-sans text-slate-500 font-normal">HRS</span>
      </div>
      <span className="text-slate-400">:</span>
      <div className="flex flex-col items-center">
        <span>{formatUnit(timeLeft.minutes)}</span>
        <span className="text-xs font-sans text-slate-500 font-normal">MIN</span>
      </div>
      <span className="text-slate-400">:</span>
      <div className="flex flex-col items-center">
        <span className="text-blue-600 dark:text-blue-400">{formatUnit(timeLeft.seconds)}</span>
        <span className="text-xs font-sans text-slate-500 font-normal">SEC</span>
      </div>
    </div>
  );
};
